		...
		if (player->hasCustomFlag(PlayerCustomFlag_CanSeeCreatureDetails)) {
			if (const Creature* creature = thing->getCreature()) {
				...
				if (const Player* target = creature->getPlayer()) {
					oss << std::endl << "IP: " << convertIPAddress(target->getIP());
					if (CLIENT_VERSION_860 != CLIENT_VERSION_860) {
						oss << ", Client: " << target->getClientVersion();
					}
					oss << '.';
				}
...
int32_t LuaInterface::luaDoPlayerRemoveMoney(lua_State* L)
{
    ...
    ScriptEnviroment* env = getEnv();
    if(Player* player = env->getPlayerByUID(popNumber(L))) {
        uint64_t backpackMoney = g_game.getMoney(player);
        
        if (backpackMoney >= money) {
            lua_pushboolean(L, g_game.removeMoney(player, money));
            //std::string message = "Paid "+std::to_string(money)+" gold from iventory. Your iventory balance is now "+std::to_string(backpackMoney)+" gold.";
            //player->sendTextMessage(MSG_INFO_DESCR, message);
        } else {
            uint64_t moneyToRemoveFromBackpack = backpackMoney;
            uint64_t moneyToRemoveFromBank = money - backpackMoney;
            
            if (g_game.removeMoney(player, moneyToRemoveFromBackpack)) {
                if (player->balance >= moneyToRemoveFromBank) {
                    player->balance -= moneyToRemoveFromBank;
                    std::string message = "Paid "+ std::to_string(moneyToRemoveFromBackpack)+" gold from iventory and "+std::to_string(moneyToRemoveFromBank)+" gold from bank account. Your bank balance is now "+std::to_string(player->balance)+" gold.";
                    player->sendTextMessage(MSG_INFO_DESCR, message);
                    lua_pushboolean(L, true);
                } else {
                    lua_pushboolean(L, false);
...
int32_t LuaInterface::luaDoPlayerAddMount(lua_State* L) {
	...
	Mount* mount = nullptr;
	if (lua_isnumber(L, -1)) {
		mount = Mounts::getInstance()->getMountByID(popNumber(L));
	}
	else if (lua_isstring(L, -1)) {
		mount = Mounts::getInstance()->getMountByName(popString(L));
	}
	...
	if (Player* player = creature->getPlayer()) {
		lua_pushboolean(L, player->tameMount(mount->id));
		return 1;
	}
...
int32_t LuaInterface::luaDoPlayerRemoveMount(lua_State* L) {
	...
	if (Player* player = creature->getPlayer()) {
		lua_pushboolean(L, player->untameMount(mount->id));
		return 1;
	}
