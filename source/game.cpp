	...
	uint32_t tradePlayerId, uint16_t spriteId)
{
	...
	if(houseTile && houseTile->getHouse() && !houseTile->getHouse()->isInvited(player) && player->getGroupId() < 6)
	{
...
void Game::playerToggleMount(uint32_t playerId, int mount)
{
	Player* player = getPlayerByID(playerId);
	if (!player) {
		return;
...
bool Game::playerSay(uint32_t playerId, uint16_t channelId, MessageClasses type, const std::string& receiver, const std::string& text)
{
	...
	if (channelId == CHANNEL_HELP)
	{
		...
		if (seconds > 0)
		{
			...
			sec << "Voce foi mutado por " << seconds << " segundos no Help Channel.";
	...
	if(muted && mute)
	{
		...
		{
			char buffer[75];
			sprintf(buffer, "Voce foi mutado por %d segundos.", muted);
			player->sendTextMessage(MSG_STATUS_SMALL, buffer);
		}
		else
			player->sendTextMessage(MSG_STATUS_SMALL, "Voce está mutado permanentemente.");
