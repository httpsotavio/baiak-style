...
void Player::onCreatureAppear(const Creature* creature)
{
	...
	for(int32_t slot = SLOT_FIRST; slot < SLOT_LAST; ++slot)
	{
		...
		g_moveEvents->onPlayerEquip(this, item, (slots_t)slot, false, true);
	}
...
void Player::onCreatureDisappear(const Creature* creature, bool isLogout)
{
	...
	for(int32_t slot = SLOT_FIRST; slot < SLOT_LAST; ++slot)
	{
		...
		g_moveEvents->onPlayerDeEquip(this, item, (slots_t)slot, false, true);
	}
...
ReturnValue Player::__queryAdd(int32_t index, const Thing* thing, uint32_t count, uint32_t flags, Creature*) const
{
	...
	if(ret == RET_NOERROR)
	{
		...
		if(!g_moveEvents->onPlayerEquip(self, const_cast<Item*>(item), (slots_t)index, true, false))
	...
	int32_t index, CylinderLink_t link /*= LINK_OWNER*/)
{
	if(link == LINK_OWNER) //calling movement scripts
		g_moveEvents->onPlayerEquip(this, thing->getItem(), (slots_t)index, false, false);
...
{
	if(link == LINK_OWNER) //calling movement scripts
		g_moveEvents->onPlayerDeEquip(this, thing->getItem(), (slots_t)index, isCompleteRemoval, false);
...
bool Player::onKilledCreature(Creature* target, DeathEntry& entry)
{
	...
	if(!targetPlayer->hasAttacked(this) && target->getSkull() == SKULL_NONE
		&& targetPlayer != this)
	{
		if (!(isInRealCastleArea(targetPlayer->getPosition())) && (addUnjustifiedKill(targetPlayer, !enemy.war) || entry.isLast())) {
...
bool Player::isInRealCastleArea(const Position& pos)
{
    Position realCastleFromPos(3308, 251, 9);
    Position realCastleToPos(3412, 371, 9);

    bool inArea = pos.x >= realCastleFromPos.x && pos.x <= realCastleToPos.x &&
    pos.y >= realCastleFromPos.y && pos.y <= realCastleToPos.y &&
    pos.z == realCastleFromPos.z;
...
bool Player::addUnjustifiedKill(const Player* attacked, bool countNow)
{
	if(!g_config.getBool(ConfigManager::USE_FRAG_HANDLER) || hasFlag(
		...
		PlayerCustomFlag_NotGainSkull) || isInRealCastleArea(attacked->getPosition()) || attacked == this)
		return false;
...
{
	std::string value;
	if (getStorage(std::to_string(PSTRG_MOUNTS_CURRENTMOUNT), value)) {
...
{
	setStorage(std::to_string(PSTRG_MOUNTS_CURRENTMOUNT), std::to_string(mountId));
}
...
bool Player::toggleMount(bool mount)
{
	...
	if (mount) {
		...
		if (!group->getAccess() && _tile->hasFlag(TILESTATE_PROTECTIONZONE)) {
...
bool Player::tameMount(uint8_t mountId)
{
	...
	int32_t value;
	std::string strValue;
	if (getStorage(std::to_string(key), strValue)) {
	...
	setStorage(std::to_string(key), std::to_string(value));
	return true;
}
...
bool Player::hasMount(const Mount* mount) const
{
	...
	int32_t value;
	std::string strValue;
	if (!getStorage(std::to_string(PSTRG_MOUNTS_RANGE_START + (tmpMountId / 31)), strValue))
		return false;
