...
function legendLever:random(cid, obj, item)
	...
	if not doPlayerRemoveItem(cid, itemId, 1) then
		...
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_RED, 'Voc� n�o possui 1x ' .. getItemInfo(itemId).name .. ' para tentar a sorte.')
		return true
	end
				...
				local position = obj.winPosition
					...
					if dummy ~= 0 then
						...
						if owner then
							...
							doPlayerSendTextMessage(owner, MESSAGE_INFO_DESCR, 'Parab�ns, voc� encontrou ' .. rewardStr .. '.')
							if not obj.discardLog then
						...
						if foundRare then
							if addItem and owner then
								doItemSetAttribute(addItem, 'description', ('[Legendary Lever]: item of player %s.'):format(getCreatureName(owner)))
							end
							...
							if owner then
								...
								doBroadcastMessage('[Legendary Lever]: O jogador ' .. getCreatureName(owner) .. ' encontrou um ' .. itemName .. ', que incr�vel.', MESSAGE_EVENT_ADVANCE)
							end
							obj.wonLegend = true
