...
function convertTime(a)
  if (type(tonumber(a)) == "number" and a > 0) then
    if (a <= 59) then
      return a .. ((a > 1) and " segundos" or " segundo")
    elseif (a <= 3599) then
      ...
      if (second == 0) then
        ...
        return ((minute ~= 0) and ((minute > 1) and minute .. " minutos e " or minute .. " minuto e ") or "") .. ((second > 1) and second .. " segundos" or second .. " segundo")
      end
    else
      local day = math.floor(a / 86400)
      local hour = math.floor((a % 86400) / 3600)
      local minute = math.floor((a % 3600) / 60)
      local second = a % 60

      local parts = {}
      if (day > 0) then
        table.insert(parts, day .. ((day > 1) and " dias" or " dia"))
      end
      if (hour > 0) then
        table.insert(parts, hour .. ((hour > 1) and " horas" or " hora"))
      end
      if (minute > 0) then
        table.insert(parts, minute .. ((minute > 1) and " minutos" or " minuto"))
      end
      if (second > 0) then
        table.insert(parts, second .. ((second > 1) and " segundos" or " segundo"))
      end
...
local block_area = {
	{{x = 14769, y = 15024, z = 7}, {x = 14785, y = 15038, z = 7}}
}
