...
realTeamBattle = {
	...
	teleportPosition = {x= 165, y= 48, z= 7},
	exit = {x = 157, y = 50, z = 7},
	ips = {}
...
function realTeamBattle.winEvent(team,ammount)
	...
	for _, pid in pairs(realTeamBattle.getPlayers()) do
		if getPlayerStorageValue(pid,realTeamBattle.teamStorage) == team then
			doPlayerAddItem(pid, 6527, 40)
			doPlayerSendTextMessage(pid,25,"[Team Battle] Parabéns por vencer o evento, você ganhou 40 Event Coins!")
		end
		realTeamBattle.cleanPlayer(pid)
		doTeleportThing(pid,realTeamBattle.exit)
	end
	...
	doBroadcastMessage("[Team Battle] "..ammount.." jogadores do time "..teams[team].." venceram o evento e ganharam 20 Event Coins!")
end
...
function realTeamBattle.empate(team1,team2)
	...
	setGlobalStorageValue(realTeamBattle.checkStorage,-1)
	realTeamBattle.createStones()
	doBroadcastMessage("[Team Battle] O evento empatou ficando apenas um jogador do time "..teams[team1].." e um do time "..teams[team2]..". Cada um deles ganhou 20 Event Coins!")
end
