...
function onStepIn(cid, item, position, lastPosition, fromPosition, toPosition, actor)
    ...
    if getPlayerGuildId(cid) <= 0 or not getPlayerGuildId(cid) or getPlayerGuildId(cid) == false or getPlayerLevel(cid) < 25 then
        ...
        doPlayerSendCancel(cid, "Voce nao tem guild ou é level menor que 25 e nao pode entrar no castelo!")
        return true
    end
    ...
    if getGlobalStorageValue(18219113) == -1 then
        if getGlobalStorageValue(1821999) ~= getPlayerGuildId(cid) then
            doTeleportThing(cid, fromPosition)
            doPlayerSendCancel(cid, "Sua guild nao é dona do castelo e o evento nao esta aberto")
            return true
        end
    end
    return true
