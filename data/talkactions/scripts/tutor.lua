		...
		local pid = getPlayersOnline()
	  		for i = 1, #pid do
				...
				elseif forbiddenLink == "donate" or forbiddenLink == "doação" or forbiddenLink == "points" then
					doPlayerSendChannelMessage(pid[i], "", "Tutor [999]: Para fazer uma doação (donate) para o nosso Servidor, Voce precisa ir no nosso WebSite http://baiak-frost.com logar sua conta e clicar no botão (Donate). Nossas promoções: Acima de 20 Reais receba o Dobro, Acima de 100 Reais receba o Triplo.", TALKTYPE_CHANNEL_O, CHANNEL_HELP)
				
				elseif forbiddenLink == "staff" or forbiddenLink == "adm" or forbiddenLink == "admin" then
					doPlayerSendChannelMessage(pid[i], "", "Tutor [999]: Nossos ADM's se encontram ausente no momento. Para recorrer a um ADM, entre no nosso grupo do Discord, ou no do Whatssap, todos os links estão no nosso site http://baiak-frost.com.", TALKTYPE_CHANNEL_O, CHANNEL_HELP)
				
				elseif forbiddenLink == "task" or forbiddenLink == "missão" then
					doPlayerSendChannelMessage(pid[i], "", "Tutor [999]: Voce encontra todas as missões no teleport Tasks que se encontra no templo ao lado do NPC Premium Points. Ao fim de todas as missões do NPC Task, Voce tem a oportunidade de obter um Items, e ate mesmo experiência.", TALKTYPE_CHANNEL_O, CHANNEL_HELP)
				
				elseif forbiddenLink == "site" then
					doPlayerSendChannelMessage(pid[i], "", "Tutor [999]: WebSite do servidor e http://baiak-frost.com.", TALKTYPE_CHANNEL_O, CHANNEL_HELP)
				...
				elseif forbiddenLink == "castle" then
					doPlayerSendChannelMessage(pid[i], "", "Tutor [999]: O Castle48H abre todos os dias as 21:00 ate as 22:30, a guild que permanecer com o domínio do Castle ate o termino terá 24HRs de Hunts Privadas.", TALKTYPE_CHANNEL_O, CHANNEL_HELP)
				...
				elseif forbiddenLink == "bug" or forbiddenLink == "bugado" then
					doPlayerSendChannelMessage(pid[i], "", "Tutor [999]: Encontrou um bug? reporte Via Whatsapp ou Via Discord. Voce tambem pode abrir um ticket em nosso WebSite que corrigiremos em no maximo 24 horas! http://baiak-frost.com.", TALKTYPE_CHANNEL_O, CHANNEL_HELP)
