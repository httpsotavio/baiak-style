...
local function removeFromList(cid, name)
    ...
    elseif getPlayerStorageValue(cid, 04420061) == itemid then
        return doPlayerSetStorageValue(cid, 04420061, -1)    
    end
    return false
end
...
return false
    ...
    if param == "" then
        ...
        local fy = not isPremium(cid) and "Não disponível para free account" or getPlayerStorageValue(cid, 04420061) ~= -1 and getItemNameById(getPlayerStorageValue(cid, 04420061)) or ""
    ...
    if t[1] == "power" then
        if getPlayerStorageValue(cid, 04421001) == 1 then
            return doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_ORANGE, "Você já está com o autoloot ativado.")
        end
    elseif t[1] == "gold" then
        if getPlayerStorageValue(cid, 04421011) == 1 then
            return doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_ORANGE, "Você já está com coleta de dinheiro ativado.")
        end
    ...
    elseif t[1] == "add" then
        if ExistItemByName(t[2]) then
            ...
            if isPremium(cid) then
                if getPlayerStorageValue(cid, 04420011) < 4 then
                    ...
                    doPlayerSendCancel(cid, "Sua lista já está cheia! Você deve remover algum antes de adicionar outro.")
                end
            else
    ...
    elseif t[1] == "list" then
        ...
        local fy = getPlayerStorageValue(cid, 04420061) ~= -1 and ""..getItemNameById(getPlayerStorageValue(cid, 04420061)).."\n" or ""
